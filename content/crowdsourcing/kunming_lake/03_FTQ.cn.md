...
本期验证实战任务将帮助学习香山昆明湖架构中FTQ模块的设计，FTQ是取值目标队列，在处理器前端设计中承担着重要作用，负责前端分支预测单元BPU和取指令单元IFU之间的交互，以及前端与后端之间交互。
...
根据任务的难度和各位的完成情况，各位将能获得数额不等的奖金。此外，如果为IFU模块找出bug并反馈确认后，有机会得到更多奖金。
...
|            | FTB项存储子队列  | ftb_entry_mem存储自分支预测结果中的ftb项     |

预期难度：3/10

参考文档：见FTQ文档中FTQ子队列章节
...
|                | 转发预测结果重定向 | 如果BPU的分支预测结果出错，需要将预测结果重定向信息转发给IFU帮助        |
|                | 修正FTQ指针   | BPU预测结果入队，或者预测结果重定向时，会修改相应的bpuPtr，以及ifuPtr |

预期难度：6/10

参考文档：见FTQ文档中BPU进入FTQ章节
...
|               | 修改发送状态队列        | 成功向IFU发起发送请求，FTQ会将状态队列设置为sent         |
预期难度：6/10
...
| 任务         | 功能点          | 描述                           |
| ---------- | ------------ | ---------------------------- |
| IFU写回预译码信息 | 写回ftq_pd_mem | IFU预译码结果将被写入FTQ的预译码子队列       |
...
| 任务   | 功能点        | 描述               |
| ---- | ---------- | ---------------- |
| 发往后端 | 将取值目标发给后端  | 后端需要存储来自FTQ的存储信息 |
...
| 任务      | 功能点            | 描述                          |
| ------- | -------------- | --------------------------- |
| 从IFU重定向 | 生成IFU重定向信息接口信息 | IFU重定向同样需要整合到一个接口中，方便之后进行转发 |
...
*pc取值目标会发给后端pc mem进行存储，使其能在本地取PC*
...
| 任务      | 功能点        | 描述                                 |
| ------- | ---------- | ---------------------------------- |
| 从执行单元写回 | 修改状态队列     | 根据后端的实际执行结果修改FTQ状态队列               |
...
| 任务    | 功能点       | 描述                      |
| ----- | --------- | ----------------------- |
| 发往BPU | 转发重定向信息   | 转发来自IFU或者后端的重定向信息       |
|       | 提交指令块     | 判断指令块是否能够提交             |
