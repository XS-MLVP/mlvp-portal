...
在本期验证文档众包任务中，各位朋友将走近香山昆明湖架构的CPL2模块，了解香山L2Cache的设计思路，加深对RISCV指令集架构的理解。欢迎各位报名参加（报名填写[这里](https://www.wjx.top/vm/wAGPNJ8.aspx)，qq群：**1038700912**。）！
...
RXSNP把来自总线B的请求进行处理，转化为内部任务格式，然后发送给ReqArb。RXRSP模块接受来自RXRSP通道的无数据的响应消息，直接把响应送到MSHRCtl，用消息中txnID用来识别mshrID。RXDAT模块接受来自RXDAT通道的有数据的响应消息，将数据存入RefillBuffer，同时把响应送到MSHRCtl，用消息中txnID用来识别mshrID。
