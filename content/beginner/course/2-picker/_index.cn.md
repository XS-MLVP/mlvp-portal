...
从这次课程开始，我们将使&#x7528;**&#x20;Python&#x20;**&#x4F5C;为芯片验证的高级语言，相关的示例也会用 Python 编写。
...
```bash
sudo apt install -y build-essential git cmake python3-venv time lcov # lcov 用来生成覆盖率报告
```
...
Picker 也提供了异步环境下的时钟方法：

* `AStep(cycle: int)`：异步等待时钟经过 cycle 个周期， 例如：`await dut.AStep(5)`

* `ACondition(condition)`：异步等待条件为真，注意`condtion`是一个返回布尔值的函数对象。
