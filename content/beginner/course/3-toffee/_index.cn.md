...
3. **返回值/`yield`**：Fixture 函数通过 `return`（或 `yield`，用于需要在测试后执行清理逻辑的情况）来提供准备好的资源。
...
> **重要提示**：默认情况下，每个测试用例**都会**获得由 Fixture **重新执行**并返回的**独立实例**。这意味着 `test_with_ref` 使用的 `dut` 实例与另一个测试用例 `test_another(dut)` 使用的 `dut` 实例是不同的对象，它们之间状态隔离，确保了测试的独立性。
...
通过使用 Fixture，我们可以有效地组织测试环境的准备工作，让测试用例本身更简洁、更专注于验证逻辑。关于 Fixture 的作用域（如 `session`、`module` 级别共享）和 `yield` 的用法，可以查阅 pytest 和 python 的相关文档。
...
使用 `async def` 定义函数和 `await` 调用之所以能实现并发（或异步执行）效果，其核心机制在于**事件循环**和协程的**协作式调度**。

工作流程如下：

1. **`async def` 标记可暂停**：`async def` 标记的函数是一个协程，表明它内部可能包含耗时或需要等待的操作，并且它有能力在这些点**暂停**执行。

2. **`await` 触发潜在的暂停与执行权让出：**`await` 关键字标志着协程中**可能**发生暂停的位置。当 `await` 一个无法立即完成的异步操作时（例如，等待像硬件时钟步进 `dut.AStep(1)` 这样的事件、I/O 操作或显式 `sleep`），当前协程会在此处暂停，并将执行权交还给事件循环，以便事件循环可以调度执行其他就绪的任务。需要注意的是，并非所有 `await` 都会导致立即的执行权切换；执行首先会进入被 `await` 的目标，只有当其内部遇到需要等待的阻塞操作时，才会发生实际的暂停和切换。

3. **事件循环接管调度：**&#x4E8B;件循环是后台的核心调度器。它回收执行权后，会检查并调度其他已就绪（未处于等待状态）的协程任务在当前线程上运行。

4. **切换到其他任务**：事件循环选择一个就绪的协程并执行它。这就是并发发生的地方——线程没有因等待而阻塞，而是被用来执行其他可运行的任务。
...
这张图展示了两个协程（代码块 A 和 B）的执行流。宏观时间上看（纵轴），A 和 B 似乎并行。微观上（横向箭头），当 A 需要等待事件（如 I/O）时，它通过 `await` 让出控制权，事件循环（图中未画出）将执行权交给 B。当 B 也遇到等待时，可能又会切换回 A（若 A 的事件已完成）。这种协作式切换是协程并发的核心。
...
1. 封装 FIFO 的端口：
   ...
   1. 创建 `WriteBundle` 类，包含写入相关的端口

   2. 创建 `ReadBundle` 类，包含读取相关的端口

   3. 创建 `InternalBundle` 类，用于封装 FIFO 内部状态信号
...
5. 使用封装好的 Bundle 环境，编写测试用例`test_full_empty`：
   ...
   1. 先将 fifo 装满后，判断`full_o`信号是否为 1

   2. 再清空 fifo，判断`empty_o`信号是否为 1

   3. 执行清空 fifo 时，检查出队序列是否与入队序列一致
...
* 使用 `await self.bundle.step()` （或 `await self.monitor_step()`） 来等待时钟周期。
...
```python
adder_agent.start_monitor("monitor_sum", 10) # 指定消息队列的大小
```
...
* **检查队列大小：** 可以使用 `monitor_size("method_name")` 获取当前队列中消息的数量。&#x20;
...
[ 同步FIFO设计规范](https://acs-frontier.feishu.cn/docx/Zd01dIfgeoIk35x2AFfc2K5AnCh?from=from_copylink)

## &#x20;本节目标

本小节旨在**帮助验证新人学习如何快速阅读和理解芯片或硬件设计规范（spec）文档**，并从中提炼出验证工作的重点。通过以 `SyncFIFO` 模块为例，带领大家逐步掌握：
...
**所谓的五步，实际上就是从五个维度去理解验证文档，这需要我们在阅读前就带上这五个问题，一个一个去文章中寻找答案，这样，你就可以把文档的阅读理解转化成五个题目，以做题的方式去理解文档，当你确保自己能答对这几个题的时候，你对硬件的规范设计就能称得上了然于胸了。**

## Step1： 理解模块的作用和目的

* **=模块是什么？它想要实现什么？**

  这一步的作用在于，让我们建立对于模块的全景认识
...
* 具体怎么做？
  ...
  注：这种分类方式并不是必须的，你可以按照个人喜好去分，比如读写使能信号可以归类与控制信号中



  * 好了，到现在我们已经解决了第一个问题，弄清楚每个输入输出信号的含义了，接下来，我们要搞清楚输入输出信号之间的控制或者说协同关系，
...
* 具体怎么做？
  ...
  * 从这里开始，我们就要具体的理解功能点了（参照 FIFO 的 `功能描述` ），它们对应了模块内部的具体逻辑，其实对于一个模块而言，功能无非就是由**顶层信号的模块控制功能，模块更新功能，模块读取功能**这几部分组成
    ...
    * 模块更新功能：
      ...
      * 内部状态更新：由输入信号（包括控制信号）导致的内部状态信号更新，比如写入操作会导致计数器变更（对应**计数器**功能）。读或者写会导致**指针更新**功能的实施。
  ...
  * 其他：除了这基本的几类，还有一些其他类型的功能，比如**时序功能**
...
* 具体怎么做？
  ...
  * 常见的边界条件有以下类型：
    ...
    * 交互边界：多个信号同时发生作用时，模块是否能正确执行？
      ...
      1. `we_i = 1 & re_i = 1` 时模块行为是否对称？（典型场景：同时读写的 FIFO）
...
之前的四步已足以让你完全理解整个模块了，接下来这一步，要求我们在此基础上，思考怎么建立起一个大致的验证计划了

* 具体怎么做？

  * 这一步要做的，其实就是在功能点的基础上细分测试点，并给出相应的覆盖点了。`FIFO`案例中，我们直接给出了`功能点和测试点` 这一节，很值得参考。

  * 关于如何划分测试点，我们在[ 入门第一讲——芯片验证基础知识（0.75 课时）](https://acs-frontier.feishu.cn/docx/VYTldVLz3ou5j7xBCm5cKnTvnDd?from=from_copylink)有详细阐述。
...
功能覆盖率模型是**验证计划中功能覆盖率目标的具体代码实现**。它不是一个抽象的概念，而是你在验证环境（例如使用 Toffee 框架）中编写的一组代码结构，其目的是：
...
4. **量化验证进度：** 通过统计有多少预定义的事件被观察到，来衡量验证对设计功能的覆盖程度。
...
**目标：** 功能验证的目标通常是驱动功能覆盖率达到 100%，即所有在覆盖模型中定义的 `CovBin` 都至少被命中一次。任何未被覆盖的 Bin 都指示了验证的“盲点”，提示我们需要补充新的测试用例或调整现有的激励生成策略来覆盖这些缺失的场景。
...
3. **动态反馈机制**：覆盖率结果应能实时反馈至测试策略，指导后续激励生成。
...
* 确保你的 FIFO Agent 和 Bundle 代码是可用的。
