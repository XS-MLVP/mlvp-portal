...
3. 收集 Bug 和覆盖率：此阶段将运行已开发的测试用例，识别并记录设计中出现的缺陷，同时收集代码覆盖率和功能覆盖率数据以评估验证的完备程度。这是一个包含调试和分析的迭代过程。
3. 进行回归测试：在设计代码发生变更（如缺陷修复或功能更新）后，重新运行相关测试用例，确保修改的正确性且未引入新问题。
3. 撰写验证报告：在验证达到特定节点或结束时，总结整个验证过程、测试结果、缺陷状态、覆盖率达成情况及存在的风险，为项目决策提供依据。
...
3) **确定组织方法**
   ...
   * **自上而下**
     ...
     2. **优点：**&#x66F4;好地把握系统级功能和性能，可在早期进行，强调用户视角。
...
- [为什么要、为什么能进行开源分包验证](https://open-verify.cc/learningresource/viewpoint/com_001/)
